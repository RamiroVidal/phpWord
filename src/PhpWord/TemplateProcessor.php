...
class TemplateProcessor
{
   ...
   public function setImageValue($search, $img, $imgSource){
	...
	public function replaceToTable($replace, $replacement, $limit=self::MAXIMUM_REPLACEMENTS_DEFAULT){
        ...
        $this->tempDocumentFooters = $this->setValueForPart($search, $replace, $this->tempDocumentFooters, $limit);
		 ...
		 preg_match(
            '/(<\?xml.*)(<w:p.*>\${' . $replace . '}<\/w:.*?p>)(.*)(<w:p.*\${\/' . $replace . '}<\/w:.*?p>)/is',
            $this->tempDocumentMainPart,
            $matches
        );
		...
		/*
		print " Replace: $replace <br> Replacement: $replacement <br><br>";
		 
		print "document: <br>" . $this->tempDocumentMainPart;
		exit();
